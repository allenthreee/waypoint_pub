...
class AgentInit:
    def __init__(self, dt=0.01, use_gazebo=True):
        ...
        self.targetPosition = PositionTarget()  # PositionTarget mode
        ...
        self.positionTarget_pub = rospy.Publisher('mavros/setpoint_raw/local', PositionTarget, queue_size=10)
    ...
    def set_target_position(self, xyzYaw):
        ...
        pose.pose.position.x = xyzYaw[0]
        pose.pose.position.y = xyzYaw[1]
        pose.pose.position.z = xyzYaw[2]
    ...
    def pub_target_position(self):
        # print("call pub_target_position()")
    ...
    def pub_target_position_small(self):
        """Publish PoseStamped with velocity and yaw rate limits"""
        # Create target pose
        self.target_pose = self.set_target_position(self.target_xyzYaw)
        
        # Position targets
        small_target_pose = PoseStamped()
        small_target_pose.pose.position.x = self.target_xyzYaw[0]
        small_target_pose.pose.position.y = self.target_xyzYaw[1]
        small_target_pose.pose.position.z = self.target_xyzYaw[2]
        
        # Calculate time step (or use fixed dt=0.2)
        current_time = rospy.Time.now()
        dt = 2
        self.last_pub_time = current_time
        
        # Velocity limiting (in position controller)
        position_error = np.array([
            small_target_pose.pose.position.x - self.uav_states[0],
            small_target_pose.pose.position.y - self.uav_states[1],
            small_target_pose.pose.position.z - self.uav_states[2]
        ])
        
        # Scale down if velocity would exceed limits
        req_velocity = np.linalg.norm(position_error) / dt
        if req_velocity > self.max_linear_vel:
            scale_factor = self.max_linear_vel * dt / np.linalg.norm(position_error)
            position_error *= scale_factor
        
        # Apply limited position error
        small_target_pose.pose.position.x = self.uav_states[0] + position_error[0]
        small_target_pose.pose.position.y = self.uav_states[1] + position_error[1]
        small_target_pose.pose.position.z = self.uav_states[2] + position_error[2]
        
        # Yaw handling with rate limiting
        target_yaw = self.target_xyzYaw[3]
        small_target_yaw = target_yaw
        current_yaw = self.uav_states[8]  # euler[2] is yaw
        
        # Calculate maximum allowed yaw step
        max_yaw_step = self.max_angular_vel * dt
        yaw_diff = target_yaw - current_yaw
        
        # Normalize angle and apply rate limit
        yaw_diff = (yaw_diff + np.pi) % (2 * np.pi) - np.pi  # [-π, π]
        if abs(yaw_diff) > max_yaw_step:
            yaw_diff = np.sign(yaw_diff) * max_yaw_step
        
        # Convert limited yaw to quaternion
        cy = np.cos((current_yaw + yaw_diff) * 0.5)
        sy = np.sin((current_yaw + yaw_diff) * 0.5)
        small_target_pose.pose.orientation.x = 0.0
        small_target_pose.pose.orientation.y = 0.0
        small_target_pose.pose.orientation.z = sy
        small_target_pose.pose.orientation.w = cy
        
        # Publish
        if not rospy.is_shutdown():
            self.pos_cmd_pub.publish(small_target_pose)
            print(f'x:{small_target_pose.pose.position.x:.2f}, y:{small_target_pose.pose.position.y:.2f}, z:{small_target_pose.pose.position.z:.2f}')


    def reach_target_position(self):
        rospy.loginfo(f"Moving to target position: {self.target_xyzYaw}")
        self.target_pose = self.set_target_position(self.target_xyzYaw)
        ...
        while not rospy.is_shutdown():
            ...
            distance = np.sqrt((current_pos.x - self.target_xyzYaw[0])**2 +
    ...
    def run(self, attitude_controller, simulation_time: float):
        ...
        # self.is_record = False
        self.is_show_rviz = False
        self.set_target_position = (.0, .0, 2.5, 0)
        self.reach_target_position()
